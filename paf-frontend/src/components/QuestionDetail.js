...
function QuestionDetail() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Header user={user} />
      <div className="max-w-4xl mx-auto p-5 bg-white shadow-md rounded-lg mt-6">
        {/* Question Header */}
        <div className="mb-5 border-b border-gray-200 pb-4">
          <h1 className="mb-2 text-2xl font-bold text-green-800">{question.title}</h1>
          <div className="flex flex-wrap gap-4 text-gray-500 text-sm items-center">
            ...
            {user && question.authorEmail === user.email && (
              <>
                <button className="text-xs text-green-600 hover:underline ml-auto" onClick={handleEditQuestion}>Edit</button>
        ...
        <div className="flex mb-8">
          <div className="flex flex-col items-center mr-5 min-w-10">
            <button
              className={`bg-transparent border-none cursor-pointer text-2xl ${question.upvotedByCurrentUser ? 'text-yellow-500' : (user ? 'text-gray-500 hover:text-yellow-500' : 'text-gray-400 cursor-not-allowed')}`}
        ...
        <div className="mt-8 mb-5 pb-2 border-b border-gray-200 flex justify-between items-center">
          <h2 id="answers-section" className="text-xl font-semibold text-green-800">
          ...
          <div className="flex items-center space-x-2">
            ...
            <select
              className="p-1 border border-gray-300 rounded-md bg-white text-gray-800 text-sm focus:ring-green-500 focus:border-green-500"
              value={answerSortBy}
        ...
        <div className="mb-8">
          ...
          {question.answers && question.answers.length > 0 ? (
            question.answers.map(answer => (
              <div key={answer.id} className="border border-gray-200 rounded-lg p-4 mb-4 bg-gray-50 flex hover:border-green-200 transition-all">
                <div className="flex flex-col items-center mr-4 min-w-8">
                  <button
                    className={`bg-transparent border-none cursor-pointer text-lg ${
                      ...
                      ? 'text-yellow-600'
                      : (user ? 'text-gray-400 hover:text-yellow-600' : 'text-gray-400 cursor-not-allowed')
                    }`}
                ...
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap items-center text-sm text-gray-500 bg-gray-100 px-3 py-1 rounded-full inline-block">
                    ...
                    {user && answer.authorEmail === user.email && (
                      <>
                        <span className="mx-1">•</span>
                        <button className="text-green-600 hover:underline" onClick={() => handleEditAnswer(answer.id)}>Edit</button>
                        <span className="mx-1">•</span>
        ...
        {user ? (
          <div>
            <h3 className="mb-4 text-lg font-semibold text-green-800">Your Answer</h3>
            ...
            <form onSubmit={handleSubmitAnswer}>
              <textarea
                ...
                rows="6"
                required
                className="w-full p-3 border border-gray-300 rounded text-base font-normal mb-4 resize-y focus:border-green-500 focus:ring-1 focus:ring-green-500"
              />
              <button
                type="submit"
                className="bg-green-700 text-white border-none py-2 px-5 rounded font-bold cursor-pointer transition-colors hover:bg-green-800"
              >
                Post Your Answer
              </button>
            </form>
          ...
          <div className="bg-green-50 p-5 text-center rounded-lg mt-5 border border-green-200">
            <p>You need to <Link to="/" className="text-green-700 font-bold hover:underline">log in</Link> to post an answer.</p>
          </div>
        )}
      </div>
    </div>
